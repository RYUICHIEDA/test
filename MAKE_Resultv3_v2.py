...
# ピーク値と血栓の位置の関係性を見るために作成
def plot_peak(arr, stlist):
    image_binary = []
    arr = np.flip(arr, axis = 0)

    for i in range(arr.shape[1]):
        # グラフをプロット
        plt.plot(stlist, arr[:, i], linestyle='-')

    # 軸ラベル（入れ替えたのでラベルも入れ替える）
    plt.title(f'x方向スキャンのピーク値 \n ファイル番号 : No.{sp_st_num}-{sp_en_num}', fontname ='MS Gothic') 
    plt.xlabel('スポット光位置 [mm]', fontname ='MS Gothic')
    plt.ylabel('ピーク値', fontname ='MS Gothic')

    plt.legend(Light_Lambda)

    #plt.show()

    # グラフの保存
    save_filename = f'{glaph_save_path}\\No{sp_st_num}-{sp_en_num}_λ{Light_Lambda[i]}_peak.png'
    plt.savefig(save_filename, bbox_inches='tight')  # bbox_inches='tight'で余白を最小限にして保存
...
#----------------------------------------------------------------------------------------------------------------------------------
# 作成したグラフを保存するファイルパス
glaph_save_path = os.path.join(os.path.dirname(folder_path), f'{ensu_num}_{sp_st_num}_{sp_en_num}')
if not os.path.exists(glaph_save_path): os.makedirs(glaph_save_path)
Excel_path = os.path.join(glaph_save_path, f'output_{ensu_num}_{sp_st_num}_{sp_en_num}.xlsx')
num_data_path = rf'C:\Users\edaryuuichi\Box\0_EDA_BOX\Report\240709\num_{ensu_num}_{sp_st_num}_{sp_en_num}.xlsx'
...
'''
steplist = make_step(LS, RS, deltax)
peak_list = np.zeros((sp_data.shape[0],sp_data.shape[1]))
out_peak_list = []

for i in range(peak_list.shape[0]):
    for j in range(peak_list.shape[1]):
        peak_list[i,j] = np.max(sp_data[i,j,:,y_center])
        out_peak_list.append([Light_Lambda[j], f'No.{i + 1}', peak_list[i, j]])

img = plot_peak(peak_list, steplist)
row = insert_images_to_excel(img,Excel_path,row,ws)

wb.save(Excel_path)

df_peaklist = pd.DataFrame(out_peak_list, columns = ['Param1', 'Param2', 'Value1'])

df_peaklist.to_excel(num_data_path, index=False)
