...
def draw_dif_colormap(arr, stlist, st, en, st_num, en_num, lamda):
    ...
    for i in range(arr.shape[0]):
        for j in range(arr.shape[2]):
            ...
            # グラフの保存
            save_filename = f'{glaph_save_path}\\Y{0-i}_No{st_num}-{en_num}_λ{lamda[j]}_差分カラーマップ.png'
...
# 輝度のカラーマップを描画するプログラム
def draw_max_colormap(arr, stlist, st, en, st_num, en_num, lamda):
    # 配列の要素の大きさに応じて色を変換する関数を定義
    cmap = plt.get_cmap('Reds')
    ...
    for i in range(arr.shape[0]):
        for j in range(arr.shape[2]):
            ...
            # グラフの保存
            save_filename = f'{glaph_save_path}\\Y{0-i}_No{st_num}-{en_num}_λ{lamda[j]}_輝度カラーマップ.png'
            plt.savefig(save_filename, bbox_inches='tight')  # bbox_inches='tight'で余白を最小限にして保存
...
start_number =  290 # ファイルの開始番号（測定では光源が測定対象中心の右側にあります）
end_number = 290    # ファイルの終了番号（測定では光源が測定対象中心の左側にあります）
...
rp_data = np.zeros((1,9,data.shape[1],data.shape[2],data.shape[3]))

for i in range(rp_data.shape[1]):
    rp_data[:,i,:,:,:] = data

scan_data = np.zeros((1,xscan_num,data.shape[1],data.shape[2],data.shape[3]))

for i in range(1):
    for j in range(xscan_num):
        scan_data[0, j, :, :, :] = rp_data[0,i * xscan_num + j, :, :, :]
...
for i in range(yscan.shape[0]):
    for j in range(yscan.shape[2]):
        yscan[i,:,j,:] = (yscan[i,:,j,:] - np.min(yscan[i,:,j,:])) / (np.max(yscan[i,:,j,:]) - np.min(yscan[i,:,j,:]))

# 差分カラーマップ描画
draw_dif_colormap(yscan, steplist, yst, yen, start_number, end_number, lamda)

# 輝度カラーマップ描画
draw_max_colormap(scan_data[:,:,:,265,yst:yen], steplist, yst, yen, start_number, end_number, lamda)