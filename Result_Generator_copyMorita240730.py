...
#---------------------------------------------------------------------------------------------------------------------------------
# ファイルのパス
folder_path = r"C:\maindata" # 000_processed.npyがあるフォルダのパスを指定してください
...
def main():
    ...
    # ターゲットを中心と仮定して、スポット光照射画像のグレースケールデータを光波長ごとに出力する
    for i in range(sp_yscan_num):
        cell = ws.cell(row = row, column = 1, value = f"y = {0 - i}")
        cell.alignment = Alignment(vertical = 'top')
        st_row = row
    ...
    for i in range(SU_yrange * 2 + 1):
        ...
        plt.xlabel("歪み度", fontname = 'MS Gothic')
        plt.ylabel("写真番号", fontname = 'MS Gothic')
