...
def gca(lst, increment):
    ...
    if lst % 2 == 1:    # リストの長さが奇数の場合
        for i in range(lst):
            centered_array.append(round((i - lst // 2) * increment, 3))
    else:               # リストの長さが偶数の場合
        for i in range(lst):
            centered_array.append(round((i - lst // 2 + 0.5) * increment, 3))
...
def process_dif(data):
    ...
    # 第二要素:光波長インデックス
    # 第三要素:yピクセル番号
    differ_min = np.zeros((data.shape[0],data.shape[1],data.shape[3]))

    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            for k in range(data.shape[3]):
                differ_min[i,j,k]=(dif_min(data[i,j,:,k],cal_ran=cal_range))
    return differ_min

# 差分のカラーマップを描画するプログラム
def draw_dif_colormap(arr, stlist, st, en, st_num, en_num, lamda):
    # 配列の要素の大きさに応じて色を変換する関数を定義
    cmap = plt.get_cmap('Reds')

    for i in range(arr.shape[1]):
        # グラフをプロット
        plt.imshow(arr[:,i,:].T, cmap=cmap, origin='lower', interpolation='nearest', aspect='auto', extent=[stlist[0], stlist[-1], en, st],norm=LogNorm())

        # 軸ラベル（入れ替えたのでラベルも入れ替える）
        plt.title(f'差分カラーマップ \n 血栓位置を中心にプロット　（血栓位置　x = 0） \n (光波長[nm]　λ = {lamda[i]}) \n ファイル番号 : {st_num} - {en_num}', fontname ='MS Gothic') 
        plt.xlabel('スポット光位置 [mm]', fontname ='MS Gothic')
        plt.ylabel('y [pixel]')

        plt.yticks(range(st, en, int((en-st)/10)))

        # カラーバー
        plt.colorbar(label='BC level')

        # グラフの保存
        save_filename = f'{glaph_save_path}\\No{st_num}-{en_num}_λ{lamda[i]}_差分カラーマップ.png'
...
# 輝度のカラーマップを描画するプログラム
def draw_lumi_colormap(arr, stlist, st, en, st_num, en_num, lamda):
    # 配列の要素の大きさに応じて色を変換する関数を定義
    cmap = plt.get_cmap('Reds')
    # グラフをプロット
    plt.imshow(arr.T, cmap=cmap, origin='lower', interpolation='nearest', aspect='auto', extent=[stlist[0], stlist[-1], en, st])

    # 軸ラベル（入れ替えたのでラベルも入れ替える）
    plt.title(f'輝度カラーマップ \n 血栓位置を中心にプロット　（血栓位置　x = 0） \n (光波長[nm]　λ = 450-600) \n ファイル番号 : {st_num} - {en_num}', fontname ='MS Gothic') 
    plt.xlabel('スポット光位置 [mm]', fontname ='MS Gothic')
    plt.ylabel('y [pixel]')
    ...
    # グラフの保存
    save_filename = f'{glaph_save_path}\\No{st_num}-{en_num}_λ450-600_輝度カラーマップ.png'
    plt.savefig(save_filename, bbox_inches='tight')  # bbox_inches='tight'で余白を最小限にして保存
...
end_number = 366    # ファイルの終了番号（測定では光源が測定対象中心の左側にあります）
...
# 要素i,j,lごとに差分の総和を格納
yscan = process_dif(data[:,:,:,yst:yen])

# 要素j（光波長）ごとにyscanをmin-maxで正規化
for i in range(yscan.shape[1]):
    yscan[:,i,:] = (yscan[:,i,:] - np.min(yscan[:,i,:])) / (np.max(yscan[:,i,:]) - np.min(yscan[:,i,:]))

# 差分カラーマップ描画
draw_dif_colormap(data[:,:,265,:], steplist, yst, yen, start_number, end_number, lamda)
'''
# 輝度分布用データ
lumi_data = process_files(output_folder,start_number,end_number, list(range(450,600,4)), xpixel, ypixel)

# 積算後格納用リスト
inte_lumi_data = np.zeros((lumi_data.shape[0],lumi_data.shape[2],lumi_data.shape[3]))

# inte_lumi_dataの光波長以外の各要素に対して波長領域の総和を格納する
for i in range(lumi_data.shape[0]):
    for j in range(lumi_data.shape[2]):
        for k in range(lumi_data.shape[3]):
            inte_lumi_data[i,j,k] = np.sum(lumi_data[i,:,j,k])
'''
norm_data = np.zeros((inte_lumi_data.shape))

# 
for i in range(inte_lumi_data.shape[0]):
    norm_data[i,:,:] = (inte_lumi_data[i,:,:]-np.min(inte_lumi_data[i,:,:]))/(np.max(inte_lumi_data[i,:,:])-np.min(inte_lumi_data[i,:,:]))
'''
# 輝度カラーマップ描画
#draw_lumi_colormap(inte_lumi_data[:,265,yst:yen], steplist, yst, yen, start_number, end_number, lamda)
max_index = np.argmax(data[1,2,:,:])
print(np.unravel_index(max_index, data[0,1,:,:].shape))
#print(data[0,1,265,:])
'''